...
public class HatchetFish extends BucketableSchoolingWaterAnimal implements GeoEntity {
	private boolean isSchool = true;

	public HatchetFish(EntityType<? extends BucketableSchoolingWaterAnimal> entityType, Level level) {
	...
	public static boolean canSpawn(EntityType<HatchetFish> entityType, ServerLevelAccessor iServerWorld, MobSpawnType reason, BlockPos pos, RandomSource random) {
		return reason == MobSpawnType.SPAWNER || iServerWorld.getLightEmission(pos) < 8 && iServerWorld.getBlockState(pos.above()).is(Blocks.WATER) && iServerWorld.getBlockState(pos).is(Blocks.WATER) && pos.getY() <= 0;
	}
