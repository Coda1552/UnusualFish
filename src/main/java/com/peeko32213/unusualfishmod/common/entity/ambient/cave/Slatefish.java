...
//REMOVE TILT FROM
public class Slatefish extends WaterAnimal implements Bucketable {
	private static final EntityDataAccessor<Boolean> FROM_BUCKET = SynchedEntityData.defineId(Slatefish.class, EntityDataSerializers.BOOLEAN);

	public Slatefish(EntityType<? extends WaterAnimal> entityType, Level level) {
	...
	public static AttributeSupplier.Builder createAttributes() {
		return Mob.createMobAttributes().add(Attributes.MAX_HEALTH, 2.0D).add(Attributes.ARMOR, 5.0);
	}
	...
	public static <T extends Mob> boolean canSpawn(EntityType<Slatefish> entityType, ServerLevelAccessor iServerWorld, MobSpawnType reason, BlockPos pos, Random random) {
		return reason == MobSpawnType.SPAWNER || iServerWorld.getBlockState(pos).is(Blocks.WATER) && pos.getY() <= UnusualFishConfig.falseSwordfishSpawnHeight;
	}

	public boolean checkSpawnRules(LevelAccessor worldIn, MobSpawnType spawnReasonIn) {
		return UnusualFishEntities.rollSpawn(UnusualFishConfig.falseSwordfishSpawnRolls, this.getRandom(), spawnReasonIn);
	}
